...
class AjaxBase
{
    ...
    public function __construct()
    {
        ...
        $this->tableNameAttributeSchema = $this->tableViewNameGenerator
    ...
    public function getArticleEventListLog(ArticleEventLogList $eventList): string
    {
        ...
        if (!empty($log[0][SoluteConfig::UM_COL_LOG])) {
    ...
    public function getAttributeSchema(): array
    {
        if (empty($this->attributeSchema)) {
            ...
            FROM 
                ...
                ";
            $this->attributeSchema = DatabaseProvider::getDb(DatabaseProvider::FETCH_MODE_ASSOC)
                ->getAll($select);
        }
    ...
    private function appendMainCategories(array $articleList): array
    {
        ...
        foreach ($articleList as $key => $article) {
            if (!empty($article[SoluteConfig::OX_COL_PARENTID])) {
                $articleList[$key][SoluteConfig::OX_COL_CATEGORY_ID]
                    = $relationList[$article[SoluteConfig::OX_COL_PARENTID]][SoluteConfig::OX_COL_CATEGORY_ID];
                $articleList[$key]['OXCATTITLE']
                    = $relationList[$article[SoluteConfig::OX_COL_PARENTID]][SoluteConfig::OX_COL_TITLE];
            } else {
                $articleList[$key][SoluteConfig::OX_COL_CATEGORY_ID]
                    = $relationList[$article[SoluteConfig::OX_COL_ID]][SoluteConfig::OX_COL_CATEGORY_ID];
                $articleList[$key]['OXCATTITLE']
                    = $relationList[$article[SoluteConfig::OX_COL_ID]][SoluteConfig::OX_COL_TITLE];
            }
        }
