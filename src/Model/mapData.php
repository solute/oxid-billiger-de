...
class mapData implements \UnitM\Solute\Core\SoluteConfig
{
    /**
     * @var int
     */
    ...
    {
        $tableNameFieldSelection = $this->tableViewNameGenerator
            ->getViewName(SoluteConfig::UM_TABLE_FIELD_SELECTION);
    ...
    {
        $tableNameObjectToAttribute = $this->tableViewNameGenerator
            ->getViewName(SoluteConfig::OX_TABLE_OBJECT2ATTRIBUTE);
        $tableNameOxattribute = $this->tableViewNameGenerator
            ->getViewName(SoluteConfig::OX_TABLE_ATTRIBUTE);
    ...
    private function collectValues(array $mappingPlan, array $articleData, array $attributeList): array
    {
        ...
        foreach ($mappingPlan as $attributeId => $mapping) {
            ...
            if (
                ...
                $data[$attributeId] = $attributeList[$fieldDefinition[SoluteConfig::UM_DR_TYPE_ATTRIBUTE][SoluteConfig::UM_DR_LABEL]];
            ...
            if (
                ...
                $data[$attributeId] = $articleData[$fieldDefinition[SoluteConfig::UM_DR_TYPE_FIELD][SoluteConfig::UM_DR_PRIMARY_FIELD]];
                $data[$attributeId] = $this->convert(
                    $data[$attributeId],
    ...
    private function getRelationFieldValue(array $relation, string $primaryId): string
    {
        ...
        $select = "
            ...
            WHERE
                ...
                AND `rt`.`" . $relation[SoluteConfig::UM_DR_RELATION_ID] . "` = `pt`.`"
            . $relation[SoluteConfig::UM_DR_PRIMARY_FIELD] . "`
        ";
