...
class FeedItem implements SoluteConfig
{
    ...
    /**
     *  if module setting is necessary, it is only need to change this value
     *
     * @var bool
     */
    ...
    private function checkEngeryClass(array $feedLine): bool
    {
        ...
        if ($checkResult['count'] > 0) {
            // check range
            $list = json_decode($this->schemaList[SoluteConfig::UM_ATTR_ENERGY_CLASS][SoluteConfig::UM_COL_VALID_VALUES], true);
            ...
            if ($worst > $best) {
                $message = 'ERROR. Minimum energy class <i>' . $feedLine[SoluteConfig::UM_ATTR_ENERGY_CLASS_MIN]
            ...
            if ($value < $worst) {
                $message = 'ERROR. Energy class <i>' . $feedLine[SoluteConfig::UM_ATTR_ENERGY_CLASS]
